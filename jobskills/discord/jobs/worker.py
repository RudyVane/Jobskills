...
async def scrape_pipeline(arq_ctx, dc_ctx, url: str):
    ...
    async with get_queue() as q:
        await q.enqueue_job("message_edit", dc_ctx, "Scraping...", {})#, _job_id=("edit_step1:{}{}".format(dc_ctx.guild_id, url)))
        scrape_job = await q.enqueue_job("scrape_handler", dc_ctx, url)#, _job_id=("scrape_handler:{}{}".format(dc_ctx.guild_id, url)))
        scrape_res = await scrape_job.result()
        await q.enqueue_job("message_edit", dc_ctx, scrape_res, {})#, _job_id=("edit_step2:{}{}".format(dc_ctx.guild_id, url)))

async def scrape_handler(arq_ctx, dc_ctx, url: str):
    async with get_queue() as q:
        scrape_job = await q.enqueue_job("scrape", url, _queue_name="arq:scraper")#, _job_id=("scrape:{}{}".format(dc_ctx.guild_id, url)))
        try:
            scrape_res = await scrape_job.result()
